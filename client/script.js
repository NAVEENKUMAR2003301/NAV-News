  ...
  fetch(
    ...
    .then((data) => {
      ...
      AllData.forEach((cEle, index) => {
        ...
        if (content) content.textContent = cEle.content.slice(0,200)+" read more..." || "No content available";
    ...
    .catch((err) => {
    //  console.error(err);
      if(load) load.textContent = `Oops! Something went wrong. Try again later. ${err}`;
    });
}
